  ...
  class UI {
    ...
    renderSummary() {
      ...
      memorial.innerHTML = `
        <div class="memorialPortrait">${getCharacterPortrait({ age: summary.age, gender: summary.gender }, "memorialPortraitImg")}</div>
        <div class="memorialName">${summary.name}</div>
        <div class="memorialYears${isBadEnd ? ' badEnd' : ''}">${t("ui.livedYears", { years: summary.age })} · ${t(causeKey)}</div>
      `;
      container.appendChild(memorial);

      // 2. Stats Grid (Money, Happiness, IQ, Best Life Trophy)
      const statsGrid = el("div", "summaryStats");
      statsGrid.style.gridTemplateColumns = "repeat(4, 1fr)";

      const bestResults = this.game.bestResults || {};
      const currentWealth = summary.finalNetWorth || 0;
      const currentScore = summary.age * 100 + Math.floor(currentWealth / 1000);
      const isBestScore = currentScore > (bestResults.highScore || 0);

      const items = [
        { iconName: "money", value: formatMoney(currentWealth) },
        { iconName: "happiness", value: `${summary.finalHappiness || 0}%` },
        { iconName: "user", value: `${summary.finalIntelligence || 0}` },
        { iconName: "trophy", value: isBestScore ? "NEW BEST" : "Best: " + (bestResults.highScore || 0).toLocaleString(), highlight: isBestScore }
      ];

      items.forEach(item => {
        const stat = el("div", "summaryStat");
        if (item.highlight) stat.style.borderColor = "#FFD700";
        stat.innerHTML = `
          <div class="statIcon">${icon(item.iconName)}</div>
          <div class="statValue" style="font-size: 14px;">${item.value}</div>
        `;
        statsGrid.appendChild(stat);
      });
      container.appendChild(statsGrid);

      // 3. Analysis Card (Mistakes & Insights)
      const analysis = summary.analysis || { mistakes: [], insight: null };

      if (analysis.mistakes.length > 0 || analysis.insight) {
        const analysisCard = el("div", "chartContainer");
        const title = el("div", "statLabel", "📉 Analysis");
        title.style.textAlign = "left";
        analysisCard.appendChild(title);

        const list = el("div", "mistakesList");

        // Insight (What If)
        if (analysis.insight) {
          const insightItem = el("div", "mistakeItem insight");
          insightItem.innerHTML = `
            <div class="mistakeIcon">💡</div>
            <div class="mistakeText">
              If you had <b>${analysis.insight.type === 'missed_investment' ? 'invested early' : 'avoided bad habits'}</b>, 
              you could have saved <b>${analysis.insight.result}</b>.
            </div>
          `;
          list.appendChild(insightItem);
        }

        // Top Mistakes
        analysis.mistakes.forEach(m => {
          const item = el("div", "mistakeItem");
          let msg = "";
          if (m.type === 'money') msg = `Burning cash cost you $${Math.abs(m.amount).toLocaleString()}`;
          else if (m.type === 'health') msg = `Neglecting health (-${Math.abs(m.amount)}) shortened your life`;
          else if (m.type === 'happiness') msg = `Unhappiness (-${Math.abs(m.amount)}) dragged you down`;

          item.innerHTML = `<div class="mistakeIcon">${icon("warning")}</div><div class="mistakeText">${msg}</div>`;
          list.appendChild(item);
        });

        analysisCard.appendChild(list);
        container.appendChild(analysisCard);
      }

      // Legacy Option (Play as Child)
      if (this.game.player && this.game.player.children && this.game.player.children.length > 0) {
        const firstChild = this.game.player.children[0];
        const legacyBtn = el("button", "btn btnPrimary btnLarge", `👑 Play as ${firstChild.name}`);
        legacyBtn.style.marginTop = "20px";
        legacyBtn.style.background = "linear-gradient(45deg, #FFD700, #FDB931)";
        legacyBtn.style.color = "#000";
        legacyBtn.onclick = () => {
          this.game.startLegacy(0);
          this.showGame();
        };
        container.appendChild(legacyBtn);
      }

      // 4. CTA Button (New Life)
      const ctaBtn = el("button", "btn btnSecondary btnLarge", "↻ New Life");
      ...
      // Revive Option (if not already revived this life?) logic handled in game or here.
      // For now, allow it if monetization exists.
      if (this.game.monetization && !isBadEnd) { // Maybe restrict revive on old age?
        const reviveBtn = el("button", "btn btnSecondary btnLarge", "?? Revive");
        reviveBtn.style.marginTop = "10px";
        reviveBtn.onclick = () => {
          this.game.monetization.showRewardedAd({
            onRewarded: () => this.game.applyReward('REVIVE')
          });
        };
